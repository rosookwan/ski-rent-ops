(() => {
  ...
  function renderMoney() {
    ...
    const actions = '<div class="pos-tabs">' + Object.entries(names).map(([kind, name]) => '<button type="button" class="so-button pos-button" data-action="pos-money-kind" data-id="' + kind + '" aria-pressed="' + (kind === d.kind) + '">' + e(name) + '</button>').join('') + '</div>';
    const balance = '<div class="pos-info-lines"><div class="pos-info-line" data-tone="' + (f.dueWon ? 'red' : 'green') + '"><strong>' + e(o.customer.name) + ' 팀</strong><span>' + e(o.receiptNo || o.id) + ' · 미수</span><b>' + e(won(f.dueWon)) + '</b></div><div class="pos-info-line"><strong>초과 수납 · 보증금</strong><span></span><b>' + e(won(f.creditWon) + ' · ' + won(f.depositHeldWon)) + '</b></div></div>';
    ...
    P.modal(names[d.kind], balance + actions + '<div class="pos-form-grid">' + input('실제로 처리한 금액 (원)', 'moneyAmount', d.amountWon, 'number', 'min="1" inputmode="numeric"') + select('처리 수단', 'moneyMethod', [['cash', '현금'], ['card', '외부 카드 단말'], ['transfer', '계좌이체']], d.method) + input('실제 결제자 (선택)', 'payer', d.payer) + (deposit ? '<div class="pos-info">보증금은 대여료와 별도 보관</div>' : select('금액 적용 대상', 'moneyScope', scope, d.scope)) + (d.kind === 'refund' || d.kind === 'deposit_out' ? input('환불·반환 사유', 'moneyReason', d.reason || '실제 반환 확인') : '') + '</div>' + errorBox(), b('취소', 'close') + b('금액 조정', 'pos-adjust', o.id) + b(names[d.kind] + ' 내역 확인', 'pos-money-review', '', 'primary'));
  }
