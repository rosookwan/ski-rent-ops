(() => {
  ...
  function card(order) {
    ...
    return P.orderCard({ name: order.customer.name, phone: order.customer.phone, tone: pending || missing ? 'orange' : members.length ? 'green' : '', badge: [label, 'grey'],
      metaParts: [pickup?.date + (pickup?.time ? ' ' + pickup.time : '') + ' 수령', pickup?.place, order.receiptNo || order.id], itemFit: 'parts', itemParts: members.length ? ['입력 ' + ready + '/' + members.length + '명'] : ['개인별 입력 없음'],
  ...
  function render() {
    ...
    return P.page('사이즈 입력 현황', '', P.cards([{ title: '오늘·내일 수령', sub: list.length + '팀', cards: list.map(card) }], { fixed: true, signature: 'size-status', empty: '규격을 준비할 접수 없음' }),
      ...
      { toolbar: O.go('준비·지급', 'preparation') + '<div class="so-actions">' + P.chip('팀별 현황', 'pos-size-status', '', true, list.length) + b('사이즈 집계', 'pos-size-summary') + '</div>', wait: missing ? '미입력 ' + missing + '명' : '없음' });
  }
