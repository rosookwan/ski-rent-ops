(() => {
  ...
  root.addEventListener('click',e=>{const b=e.target.closest('#ski-first-look button');if(!b)return;if(b.dataset.view){e.stopPropagation();go(b.dataset.view==='vehicle'?'vehicle':'intake');return;}if(['board','show-saved'].includes(b.dataset.action)){e.stopPropagation();go('dispatch');return;}if(b.dataset.action==='time-settings'){e.stopPropagation();state.tabs.settings='operations';go('settings');return;}if(b.dataset.action==='save'){e.stopPropagation();if(window.SkiOps.returnUI)window.SkiOps.returnUI.saveIntake();else root.querySelector('#ski-first-look [data-action="quote"]')?.click();return;}if(['send-sms','complete-confirm','final-confirm','customer-save','save-time-settings'].includes(b.dataset.action)||b.hasAttribute('data-alert')||b.hasAttribute('data-final')){e.stopPropagation();toast('처리 화면 예시입니다. 실제 저장·발송·상태 변경은 하지 않습니다.');}},true);
