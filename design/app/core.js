(() => {
  ...
  const managementRoutes=new Set(['management','partners','closing','customers','inventory','settings','guide']);
  const sharedRoutes=new Set(['home','lift-reservations']);
  function nav(){if(registry.get(state.page)?.pos&&window.SkiOps?.pos)return window.SkiOps.pos.navigation(activePage(),id=>registry.has(id));const management=state.workspace==='management';return '<div class="so-nav-group"><span class="so-nav-caption">'+(management?'관리 업무':'포스 업무')+'</span>'+routes.filter(([id])=>sharedRoutes.has(id)||managementRoutes.has(id)===management).map(([id,label,glyph])=>'<button type="button" class="so-nav-button" data-go="'+id+'" aria-label="'+label+'" title="'+label+'" '+(registry.has(id)?'':'disabled')+' '+(activePage()===id?'aria-current="page"':'')+'>'+icon(glyph)+'<span class="so-nav-label">'+label+'</span></button>').join('')+'</div>';}
  function activePage(){return registry.get(state.page)?.parent||state.page;}
  function workspaceFor(page,preferred=state.workspace){const config=registry.get(page),parent=config?.parent||page;if(config?.pos)return parent==='management'||config.workspace==='management'?'management':'pos';return config?.public||sharedRoutes.has(parent)?preferred:managementRoutes.has(parent)?'management':'pos';}
  function go(page,params={},replace=false,workspace=state.workspace){
    if(window.SkiPosOperating && window.SkiOps?.posData?.snapshot?.actor?.role==='driver' && !['login','dispatch','vehicle','driver-task'].includes(page)){toast('담당 차량의 업무 화면에서 처리해 주세요.');return;}
  ...
  function render(){const config=registry.get(state.page);if(!config)return;root.dataset.pos=String(!!config.pos);root.dataset.actor=window.SkiOps?.posData?.snapshot?.actor?.role||'store';root.classList.toggle('so-vehicle',state.page==='vehicle'&&!config.pos);root.classList.toggle('so-at-entry',!!config.entry);root.dataset.workspace=state.workspace;$('#so-navigation').innerHTML=nav();$('#so-navigation').setAttribute('aria-label',config.pos?'포스 업무 순서':state.workspace==='management'?'관리 업무':'포스 업무');$('.so-sidebar').setAttribute('aria-label',state.workspace==='management'?'관리 메뉴':'포스 메뉴');$('#so-breadcrumb').textContent=(state.workspace==='management'?'관리 · ':'')+config.title;$('#so-workspace').hidden=!!config.public;$('#so-public').hidden=!config.public||!!config.entry;$('#so-entry').hidden=!config.entry;
    ...
    const pageTools=$('#so-page-tools');pageTools.innerHTML=config.headerTools?.()||(config.pos?(window.SkiOps?.posData?.pending?'<button type="button" data-action="pos-retry" class="pos-search-entry">앞선 처리 다시 확인</button>':'<button type="button" data-go="rentals" class="pos-search-entry">'+icon('search')+'고객 찾기</button>'):'');pageTools.hidden=!config.headerTools&&!config.pos;
    ...
    const legacy=state.page==='intake'&&config.legacy!==false;$('#so-intake-tools').hidden=!legacy;if(!legacy)$('#so-intake-tools').replaceChildren();
    $('#so-page').hidden=legacy;$('#so-legacy').hidden=!legacy;
  ...
  function modal(title,body){$('#so-dialog-title').textContent=title;$('#so-dialog-body').innerHTML=body;if(registry.get(state.page)?.pos)window.SkiOps?.pos?.prepareModal();$('#so-dialog').showModal();icons();}
  ...
  root.addEventListener('click',e=>{const b=e.target.closest('button');if(!b||b.disabled)return;if(b.dataset.intakeAction){$('#ski-first-look').dispatchEvent(new CustomEvent('ski:intake-action',{detail:b.dataset.intakeAction}));return;}if(b.dataset.go){go(b.dataset.go,b.dataset.id?{id:b.dataset.id}:{});return;}if(b.dataset.subtab){state.tabs[state.page]=b.dataset.subtab;render();return;}if(b.dataset.action&&actions.has(b.dataset.action)){actions.get(b.dataset.action)(b.dataset.id,b);return;}if(b.id==='so-menu-toggle'){state.menuOpen=!state.menuOpen;root.classList.toggle('so-menu-open',state.menuOpen);b.setAttribute('aria-expanded',String(state.menuOpen));b.setAttribute('aria-label',state.menuOpen?'메뉴 접기':'메뉴 펼치기');}});
  ...
  actions.set('workspace-switch',()=>{if(state.workspace==='pos')go(registry.has('management')?'management':'home',{},false,'management');else{const target=state.posReturn||{page:'home',params:{}};go(target.page,target.params,false,'pos');}});
