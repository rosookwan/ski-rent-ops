(() => {
  ...
  function row(item, op, api) {
    ...
    return '<article class="pos-row pos-fulfillment-row"><div class="pos-row-copy"><strong data-fit="words" title="' + e(label(line)) + '">' + e(label(line)) + '</strong><p data-fit="parts" data-parts="' + e(JSON.stringify(note)) + '" title="' + e(note.join(' · ')) + '">' + e(note.join(' · ')) + '</p></div><div class="pos-row-actions">' + prepare
  ...
  function show(op, api) {
    ...
    const appointments = [...new Set(plans.map(plan => [plan?.date, plan?.time, plan?.place].filter(Boolean).join(' ')))].filter(Boolean);
    const summary = (order.receiptNo || order.id) + (appointments.length ? ' · ' + appointments.join(' / ') : '');
    const money = '<div class="pos-fulfillment-money"><span>미수</span><b data-tone="' + (order.finance.dueWon ? 'red' : 'green') + '">' + e(S.money(order.finance.dueWon)) + '</b>' + b('수납', 'pos-fulfillment-money', order.id) + '</div>';
    const primary = op.kind === 'return' && op.all ? '모두 반납 확정' : api.confirmLabels[op.kind];
    const extra = op.error ? b('최신 수량으로 다시 선택', 'pos-fulfillment-refresh') : op.kind === 'return' && op.all ? b('수량을 따로 선택', 'pos-fulfillment-partial') : b('수량 설정', 'pos-fulfillment-tools');
    P.modal(order.customer.name + ' · ' + names[op.kind] + ' ' + total(op) + '개', '<section class="pos-fulfillment-picker" aria-label="이번 확인 ' + total(op) + '개">' + rows + '</section>' + money + '<p class="pos-error" data-fulfillment-error role="alert"' + (op.error ? '' : ' hidden') + '>' + e(op.error || '') + '</p>',
      '<div class="pos-fulfillment-extra">' + extra + '</div>' + b('취소', 'close') + b(primary, 'pos-fulfillment-confirm', '', 'primary'), summary);
