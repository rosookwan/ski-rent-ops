...
const { chromium } = require('/Users/sookwan/Developer/ski-rent-ops/node_modules/playwright');
const path = require('node:path');
(async () => {
  const browser = await chromium.launch({ channel: 'chrome', headless: true }), page = await browser.newPage(), out = [];
  for (const [w, h] of [[1024, 600], [907, 648], [1024, 768]]) for (const show of ['s05', 's06', 's07']) {
    ...
    const r = await page.evaluate(show => {
      const root = show === 's05' ? document.querySelector('.work') : document.querySelector(show === 's06' ? '#dlg6' : '#dlg7');
      const clipped = [...root.querySelectorAll('button,dd,span,b,h2,p')].filter(el => el.getClientRects().length && el.scrollWidth > el.clientWidth + 1).map(el => el.textContent.trim().slice(0, 24) + ' ' + el.scrollWidth + '>' + el.clientWidth);
