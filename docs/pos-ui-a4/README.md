...
**상태 정정:** 아래는 `aa1dae6`의 촬영 기록이다. 업무 검사·화면 규칙은 통과했지만 사용자가 시안과의 차이를 지적했으며, 시안 일치는 미완료다. [차이의 사유와 남은 일](../51-pos-ui-design-differences.md)을 함께 읽는다. 이 그림을 시안 일치 승인본으로 취급하지 않는다.
