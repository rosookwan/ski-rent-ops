...
- **현재 배포(2026-09-22):** A1~A5에 이어 사용자 요청으로 **A6 헤더 UI `d500996`도 main·Pages에 반영했다.** 빌드·배포 성공, 공개 파일 네 개의 해시 일치, 실제 접수·재고 화면의 헤더·메뉴 배치를 확인했다. 상세 기록은 [`docs/43` 16절](43-ui-v4-implementation.md#16-a6-헤더-pages-배포-2026-09-22)이다. 실제 문자 발송 미연결·예약금 미수 보류는 그대로다.
- **현재 작업:** A6 공통 바깥 배치(전체 폭 헤더·그 아래 왼쪽 메뉴)는 사용자 배포 요청까지 반영했다. **전체 시안 일치는 미완료**이며 헤더 내부·메뉴 세부 모양의 남은 차이는 [`docs/51`](51-pos-ui-design-differences.md)의 요소별 표를 따른다. 거래처 상세 등 다음 항목은 확인 전 시작하지 않는다.
- **2026-09-23 전체 점검:** 실제 매장 업무 흐름, 화면 점검 결과, 흩어진 남은 UI 작업을 [`docs/52`](52-operations-flow-and-ui-review.md)에 모았다. **배달 접수를 차량에 싣는 버튼이 없어 배달 업무가 생기지 않고 `지급`이 매장 직접 지급으로 기록되는 결함(N1)** 을 체험판에서 재현했다. 다음 작업을 고를 때는 그 문서의 5절 추천 순서와 6절 사장님 확인 질문을 먼저 본다.

## 1. 먼저 읽을 것

| 순서 | 문서 | 왜 |
|---|---|---|
| 0 | [`docs/51`](51-pos-ui-design-differences.md) | 마지막 마무리 상태 · 업무상 차이의 사유 · A4 시안 불일치. 이전의 완료 표현보다 이 기록을 먼저 읽는다 |
| 1 | 이 문서 | 전체 그림과 남은 일 |
...
| 8 | [`docs/52`](52-operations-flow-and-ui-review.md) | 실제 업무 흐름(현장 방문 · 전화 예약 · 야간 차량 수거), 새로 찾은 문제 N1~N18, 남은 UI 작업 모음, 추천 순서, 사장님 확인 질문 |
...
시안 미리보기는 `design/higgsfield/pos-rest-v1/`에 있다. 완료 기준은 해당 업무 검사 통과 + 규칙 0건 + 1024×600에서 스크롤 없음 + 문구·동작 이름 유지 + **같은 표시 영역에서 시안의 위치·간격·크기·색·버튼 형태를 직접 비교**하는 것이다. 규칙 0건만으로 시안 일치를 판정하지 않는다. 업무상 차이와 미구현 기능·배치 차이를 [`docs/51`](51-pos-ui-design-differences.md)처럼 구분한다.
...
| A6 | 공통 바깥 배치 | `p02r-intake-cards` · `m02-inventory` | `pos-shell.css` · `capture-pos-release.cjs` | **전체 폭 헤더·그 아래 메뉴 위치 수정, 2026-09-22 사용자 요청으로 main·Pages 반영(`d500996`).** 글꼴·줄바꿈·본문 유지. 전체 검사 통과·규칙 322곳 0건, 기사 37장 변경 없음. 헤더 내부·메뉴 세부 모양과 본문의 기존 차이는 [`docs/51`](51-pos-ui-design-differences.md). 배포와 전체 시안 일치 여부는 구분한다. |
