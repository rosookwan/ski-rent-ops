...
작성일: 2026-09-07 · 기능 모듈 1.0 · UI 연결 전

## 구현과 배포 범위

반납 기능을 기존 디자인과 분리했습니다. 기존 화면·샘플 원장·결제·문자 버튼은 유지합니다. 새 버튼과 목록은 UI 개선 이후 [UI 연결 작업](11-return-ui-handoff.md)에서 추가합니다.
...
| 후속 UI | 접수/차량/매장/집계 화면을 새 기능에 연결. 기존 버튼에 자동 연결하지 않음 |
...
| `design/app/return-runtime.js` | Pages의 `window.SkiOps.returns` 연결점. UI 변경 없음 |
...
단위·통합 검증은 수량·일정·권한·정정·집계·중복 요청·동시 충돌·DB 재시작·HTTP 요청·클라이언트 실패 처리를 포함합니다. 브라우저에서는 일부 수거→의류 직접반납→리프트권 익일 계획→매장 확인→리프트권 회수→수량 정정을 실행합니다. 새로고침 초기화·외부 변경 요청 없음·기존 화면 샘플 유지도 확인합니다. 실제 현장 기기와 새 버튼의 사용성은 후속 UI/운영 연결 범위입니다.
