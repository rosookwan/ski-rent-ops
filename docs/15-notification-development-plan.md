...
작성일: 2026-09-08 · 문서 버전: 1.1 · 계획 기준 소스: `6f4f20c` · 상태: 알림 UI·API·반복 소리 구현, 운영 연결 일부와 실기기 검증 남음

매장 POS와 차량 화면에 상단 알림함을 추가한다. 기사 수거, 업무 요청과 변경을 놓치지 않도록 미확인 개수와 종 흔들림을 표시하고, 실제 기기 간 공유와 반복 알림음을 연결한다. 아래는 최초 개발 계획이며 현재 구현·검증 결과는 [알림 구현 기록](17-notification-release.md)에 구분해 기록했다. 2절의 코드 현황은 계획 작성 당시 상태다.
...
큰 기능 범위로 나누되 다음 단계는 앞 단계의 완료 기준을 통과한 후 진행한다. 아래는 목표 기준이며 현재 결과는 이어지는 구현 상태 표와 구현 기록을 따른다.
...
| 단계 | 이번 결과 |
|---|---|
| 1 | 공통 알림함·전체 행 확인·매장/차량 요청·미확인 종 구현, 로컬 화면 검사 통과 |
| 2 | SQLite 저장·원자적 처리·확인 API·전용 동기화 구현, 두 인증 HTTP 클라이언트 검증 통과. 운영 브라우저·호스팅 연결은 남음 |
| 3 | 반납 이벤트 연결과 업무 ID·배정·취소 API 구현. 화면의 직원 명단을 실제 계정에 연결하는 작업과 운영용 화면 진입점은 미완료 |
| 4 | 소리 활성화·30/60초·볼륨·단일 재생·확인 후 중지 구현, Chrome 재생 호출과 실제 타이머 검증 통과 |
| 5 | 미실시. 탭 S6 실물, 운영 서버와 시스템 푸시가 연결되지 않음 |
...
새 알림 검증 명령 `test:notifications`, `test:notifications:ui`를 추가했다. 기존 `check`, `test:returns`, `test:returns:ui`, `test:intake`, `test:time-picker`, `test:operations`, `test:visual`과 기본 클릭 흐름도 검사했다. 결과는 구현 기록에 남긴다. 소리는 자동 재생 호출 검사와 실제 기기에서 들리는지 확인한 결과를 구분한다.
