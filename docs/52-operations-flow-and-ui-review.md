...
작성 2026-09-23(같은 날 사장님 답 반영). 사용자 요청: "시스템 UI를 보고 전체적으로 더 개선해야 할 부분 체크, 아직 미진행된 UI 개선건 정리, 업무 프로세스 정리와 더 나은 방법 고민".
...
- **빈틈은 "현장에서 가장 자주 일어나는 조합"을 버튼으로 바로 고르지 못하는 곳에 몰려 있다.**
  ...
  3. **야간 수거에 쓸 '수거 목록'(포스트잇 대신)과 기사 쪽 순서 바꾸기가 없다.** 목록 인쇄, 기사 화면 검색, 기사의 순서 변경이 없다. → N3 · N12
  4. **강습 품목이 없다.** 우리 매장은 쓰지 않아 다른 매장용으로 미룬다. → N5
...
- **사장님 답 반영(2026-09-23, 6-5절):** 차가 서는 곳이 매번 다르고 조기 반납이 많아 **순서 바꾸기(기사·매장)와 `빨리 확인`(맨 위 고정)** 을 10월 핵심으로 올렸다. 리프트권은 값을 미리 다 받고(노쇼 대비), 가족 여러 팀은 마지막에 한 번에 내기도 해서 **여러 팀 한 번에 수납(N19)** 을 더했다. 남은 질문 23개는 체크만 하면 되는 [확인 질문지](https://claude.ai/artifact/5tFWna6a1K6m9Z6iroGWRE)에, 부모님 확인용 그림은 [하루 흐름](https://claude.ai/artifact/Y2Myr96ySYeFsgTCVYcWP1)에 있다(둘 다 비공개).
...
3. 리프트권 권종·수량, 강습 여부(**지금 시스템에 강습 없음** — 우리 매장은 쓰지 않고, 다른 매장용으로 2-5)
4. 결제 — 장비는 보통 카드, 리프트권은 보통 현금이라 나눠서 수납
5. 수령: 바로 가져감 또는 차량 배달(스키장 장소)
6. 반납: 매장 직접 또는 차량 수거(스키장 장소·시간). "오늘 야간까지 탄다" → 야간 차량 수거
7. 차량: 올라가기 전에 오늘 수거할 팀을 포스트잇에 적음(이름 · 연락처 · 장비 수량 · 반납 위치) → 야간에 먼저 올라가 대기(서는 곳은 그때그때 다름, 조기 반납 손님 때문에 왔다 갔다 함) → 한꺼번에 받아 내려옴 → 매장 입고
8. 정산·마감

**(B) 전화 예약**

1. 전화로 리프트권과 인원 예약
2. 단체: 사이즈를 미리 받음 → 준비표 → 전날 장비 세팅 / 소규모: 리프트권 대금만 먼저 입금(리프트권을 리조트에서 미리 끊어 두므로 노쇼 대비)
...
```mermaid
flowchart TD
  W[현장 방문<br/>대표자 · 연락처] --> F[장비 맞춤 · 품목<br/>리프트권 · 강습은 다른 매장]
  ...
  U --> X{반납}
  X -- 매장 직접 --> B[매장 반납 확인]
  X -- 차량 수거<br/>스키장 · 숙소 --> V[차량 수거<br/>야간은 대기하며 한꺼번에 · 조기 반납은 왔다 갔다]
  V --> I[매장 입고]
  B --> C[정산 · 마감]
  I --> C
...
| 숙소로 수거 | 솔마을 · 꽃마을을 구역으로, 한솔동 · 들국화 등을 장소로 등록하면 장소 창에서 두 번 눌러 고른다(구역 6개 · 1024×600 확인, 2026-09-23). 없으면 장소 `직접 입력`(글자). 구역 줄은 줄바꿈이 없어(`.pos-choice`) 7개를 넘으면 확인 필요 | 됨(설정 필요) |
...
| 기간 연장 · 교환 | `기간·수거 변경`, `문제 해결·정정 → 교환`(교환은 스키·보드 줄만, `pos-fulfillment.js:225` — 의류·헬멧도 필요, 답 7) | 됨 / 부족 (N17) |
...
| 예약금(일부 선입금) | 예약금 개념 없음(`finance.js`의 보증금과 다름). 수납으로 넣으면 카드에는 계속 `미수 N원`. 우리 매장은 리프트권 값 전부를 미리 받고, 예약금은 다른 매장용 | 없음 (N4) |
| 여러 팀 한 번에 결제 · 다른 팀이 결제 | 수납은 접수 하나씩(`finance.payment`의 `orderId` 하나) | 없음 (N19) |
| 배달 때 리프트권 추가 · 현장 수납 | 기사 자료에 접수·금액이 없음 | 없음 (N20) |
| 인원만 예약(장비 미정) | 사이즈 요청은 사람별 장비 줄이 있어야 가능 | 부족 |
| 단체 사이즈 미리 받기 | `사이즈 요청` → 입력 링크 준비(문자 실제 발송 미연결) → 준비표·팀 스티커 인쇄 | 됨(발송은 수동) |
| 강습 | 없음(`docs/08`에서 시험 범위 밖으로 뺐던 항목). 우리 매장은 쓰지 않음 — 다른 매장용(2-5) | 없음 (N5) |
| 방문 순서 변경 · 기사에게 알림 | 자료는 있음(`dispatch.reorder`, `task.priority`), 지금 포스에는 버튼 없음(예전 화면에만 있었음). 순서 변경은 매장만 허용 — 기사도 바꿔야 함(답 2) | 없음 (N12) |
...
| 설정 | 끄면 사라지는 것 | 스키노트 기본(2026-09-23 답 반영, 나머지는 질문지) |
|---|---|---|
...
| 강습 | 강습 칸 · `강습팀` 목록 · 강습 목록 | **미사용**(다른 매장용, 2-5) |
| 사이즈 미리 받기 | `사이즈 요청` 버튼 · 사이즈 입력 현황 | 사용(단체) |
| 선입금 방식 | (접수 확정 창의 처음 선택) 리프트권 값 전부 / 예약금 금액 / 받지 않음 | **리프트권 값 전부**(2-6) |
| 발권 후 취소 환불 | (취소 창의 처음 선택) 환불 / 환불 안 함 — 직원이 그때 바꿀 수 있음 | 질문지 3번 |
| 보증금 | 보증금 줄 | 질문지 1번 |
| 결제 시점 기본값 | (접수 확정 창의 처음 선택) 접수 때 / 지급 때 / 반납 때 | 접수 때(다른 팀·가족은 나중에 한 번에, 2-8) |
| 교환 가능 품목 | 교환 버튼이 나오는 품목 | **전체**(스키 · 보드 · 의류 · 헬멧) |
...
사장님 답(2026-09-23): **차가 서는 곳은 매번 다르고, 먼저 반납하려는 손님이 많아 차가 왔다 갔다 한다.** 그래서 장소로 묶기보다 **순서를 누구든 바로 바꿀 수 있는 것**이 핵심이다.

- **순서 바꾸기 — 기사와 매장 둘 다:** 기사 화면 행에 `▲ ▼ · 맨 위로`. 지금 `dispatch.reorder`는 매장만 허용한다(`common.js:64`의 `C.store`). 기사는 자기 차량 업무만 바꾸도록 `C.vehicle` 검사로 넓힌다. 바뀐 순서는 반대쪽 화면에 바로 반영한다(`docs/16` 7절).
- **`빨리 확인` — 매장에서 한 번에:** 매장으로 반납·배달 요청 전화가 오면 `차량 운행` 카드의 `빨리 확인`을 누른다. 그 업무를 기사 목록 **맨 위에 고정**하고 알린다. 이미 있는 `dispatch.reorder`의 `top`과 `task.priority`(우선 확인 요청)를 한 버튼으로 묶는 것이다. 수거와 배달 모두 같다. 기사 기기에서 소리를 낼지는 질문지 13번.
- **매장(출발 전) 수거 목록:** `차량 운행 → 수거 목록`. 날짜 · 반납 타임으로 묶고, 팀마다 포스트잇 한 줄(이름 · 연락처 · 수량 · **그 팀의 장소**)을 쓴다. 서는 곳이 바뀌므로 장소 묶음은 보조로만 쓴다. `수거 목록 인쇄`(A4 한 장, 라벨 프린터가 정해지면 팀별 라벨)와 기사 기기 전송을 같이 한다. `docs/35`에 문구(`수거 목록 인쇄` · `미반납 목록 인쇄`)가 이미 정해져 있다.
- **기사(현장):** 위에 끝 4자리 숫자판 검색, 팀마다 큰 `모두 받음` · `일부만 받음`, 받은 팀은 아래로 내리고 안 온 팀은 위에 남기며 `전화` 버튼을 행에 둔다. 촘촘한 행 목록·쪽 나눔 규칙(기사 화면 예외)은 그대로 지킨다.
- **내려와서:** `매장 입고 N개` 한 번(A5에서 구현됨) → 매장 `차량 입고 확인`.
- **시간 맞춰 알려 주기:** 야간 반납 시각(질문지 14번) 한 시간 전쯤 `오늘 할 일`의 `지금 처리`에 `야간 수거 준비 · 12팀`을 띄운다.
- **통신 대비:** 밤 주차장은 전파가 약할 수 있다. 인쇄한 수거 목록에 체크해 두었다가 내려와서 입력하는 방법을 기본 대비책으로 두고, 기기 안 임시 보관은 운영 서버 구조(`docs/34`)와 함께 따로 검토한다.

### 2-5. 강습: 물건이 아닌 품목, 매장마다 켜고 끄기

사장님 답(2026-09-23): **우리 매장은 강습을 쓰지 않는다. 다른 매장은 쓸 수 있다. 스키학교보다 타 샵이나 강습팀에 맡기는 경우가 많다.** 그래서 `사용 기능`에서 켜는 기능으로 만들고, 우리 매장은 끈다. 우리 매장의 시즌 준비 대상은 아니다.

- 매장 설정에 **`강습팀` 목록**(팀 이름 · 연락처, `우리 강사`도 한 줄)을 둔다.
- 새 접수 2단계 품목 칸에 `강습`을 넣고, 누르면 창에서 강습팀 · 종류(스키·보드) · 인원 · 시간(오전 · 오후 · 직접) · 금액을 버튼으로 고른다.
...
- 강습팀 정산은 거래처 장부(거래처 = 강습팀)를 그대로 쓸 수 있다. 거래 종류에 `강습 위탁`을 더한다.

### 2-6. 전화 예약: 칸별 수납과 선입금 방식

사장님 답(2026-09-23): **우리 매장은 리프트권 값을 대부분 미리 다 받는다.** 리프트권을 리조트에서 미리 끊어 두었다가 그대로 전달하기 때문에, 당일 많은 인원이 노쇼하면 손해가 나서다. **예약금만 받는 매장도 있을 수 있으니 매장 설정에서 고르게 한다.** 끊어 둔 권을 당일 취소할 때 환불할지도 매장 판단이다.

- **매장 설정 `선입금 방식`:** 리프트권 값 전부 / 예약금 금액 / 받지 않음. 접수 확정 창의 처음 선택이 이 값을 따른다.
...
- **예약금 방식 매장:** 칸의 `받을 금액`을 눌러 일부 금액만 받게 하고, 그 수납을 예약금으로 표시한다. 카드에는 `예약금 5만 수납 · 잔액 N원`처럼 보여 준다.
- **발권 후 취소:** 취소할 때 `리프트권 환불 / 환불 안 함`을 직원이 고른다. 처음 선택은 매장 설정 값(질문지 3·4번).
- **인원만 예약:** 새 접수에 `인원 N명 · 장비 당일` 한 줄을 허용하고, 당일 `일행·장비 추가`로 채운다.

### 2-8. 여러 팀 한 번에 수납 (2026-09-23 추가)

사장님 답: 보통 빌릴 때 받지만, **"이따가 다른 팀이 결제한다"** 는 경우가 있고, **가족 단위로 여러 팀이 따로 와서 마지막에 한 번에** 결제하기도 한다.

- 지금 수납은 접수 하나씩이다(`finance.payment`의 `orderId` 하나). 가족 여섯 팀을 카드 한 번으로 받으면 한 접수에 몰아 적게 되고, 나머지 다섯 팀은 계속 미수로 남는다.
- **`결제할 팀` 표시:** 접수 확정 창에서 `나중에`를 고를 때 결제할 팀을 고르게 한다. 그 팀의 카드에는 빨간 미수 대신 `김OO 팀 결제 예정`을 보여 준다.
- **`한 번에 수납`:** 고객 찾기나 `정산·마감`에서 여러 팀을 골라 한 번에 받는다. 기록은 팀마다 수납 1건씩, 같은 결제 묶음 번호로 남겨 팀별 미수가 정확히 0이 되게 한다. 나눠서 결제(S08R, 한 팀을 여러 번)의 반대 방향이므로 같은 창 설계에서 함께 다룬다(4-3).
- 질문지 6 · 7번(합쳐서 내는지, 몇 팀까지인지).

### 2-9. 배달 현장 수납 · 리프트권 추가 (2026-09-23 추가)

사장님 답: 가끔 **배달 가서 리프트권을 추가하는 등 돈을 받는 경우**가 있다. 미수로 두고 매장에서 받기도 한다.

- 지금 기사 자료에는 접수와 금액이 없다(`service.js`의 기사 `snapshot()`).
- 기사 화면에 **그 업무의 미수만 읽기로** 보여 주고, `현장 수납`(금액 · 수단)과 `미수로 두기`를 고르게 한다. 리프트권 추가는 차량 예비권에서 전달하고 접수에 품목으로 더한다(미수로 시작).
- A5에서 보류한 기사 화면의 `예약금 미수` 안내 띠도 이 금액 자료로 풀린다.
- 질문지 16 · 17번(현장 결제 수단, 여분 리프트권).
...
| N3 | 높음 | 차량 운행 · 기사 목록 | 포스는 차량별 카드만, 기사는 시간순 행(1024×600에서 한 쪽 4줄)만 있다. 장소·시간 묶음, 검색, 행의 전화번호(`pos-dispatch.js:37-42`), 인쇄가 없다. 한 장소 10팀이면 10번 따로 처리한다 | 2-4: 반납 타임별 `수거 목록`(팀마다 장소 한 줄 · 인쇄 · 기사 검색). 서는 곳이 매번 달라 장소 묶음은 보조, 순서는 N12 |
| N4 | 높음 | 접수 확정 창 | 전부 수납 / 전부 나중 둘 중 하나(`pos-orders.js:543`). 예약금 없음 | 2-6: 결제 수단 줄에 `나중에` 추가(높이 그대로), `받을 금액`으로 예약금 |
| N5 | 중간 | 품목 | 강습 없음. 우리 매장은 쓰지 않음(답 1) | 2-5: `사용 기능`으로 켜고 `강습팀` 목록 — 다른 매장용이라 시즌 뒤 |
...
| N12 | 높음 | 차량 운행 · 기사 목록 | 방문 순서 `▲ ▼` · `시간순 되돌리기` · `기사에게 알리기`, 우선 확인 요청이 없다. 자료는 있다(`dispatch.js:17`, `:82`), 예전 화면(`workflow-ui.js:166-169`)에만 있었다. 순서 변경은 매장만 허용(`common.js:64`의 `C.store`) | 2-4: 기사 · 매장 양쪽 `▲ ▼ · 맨 위로`, 매장 `빨리 확인` = `dispatch.reorder`의 `top` + `task.priority`를 한 버튼으로. 배달 · 수거 공통(답 2) |
...
| N17 | 중간 | 교환 | 스키·보드 줄에만 `교환` 버튼(`pos-fulfillment.js:225`). 의류·헬멧 사이즈 교환 없음 | 의류 · 헬멧도 교환(답 7). 교환 창의 규격 선택을 품목별로 |
...
| N19 | 높음 | 수납 | 수납이 접수 하나씩이라, 가족 여러 팀을 카드 한 번으로 받으면 한 접수에 몰아 적게 되고 나머지 팀은 미수로 남는다. 다른 팀이 결제할 팀도 빨간 미수로 보인다 | 2-8: `결제할 팀` 표시 + `한 번에 수납`(팀마다 수납 1건 · 같은 묶음 번호) |
| N20 | 중간 | 기사 업무 처리 | 기사 자료에 금액이 없어 배달 현장의 리프트권 추가 · 수납을 기록할 수 없다(`service.js`의 기사 `snapshot()`) | 2-9: 그 업무의 미수만 읽기 표시, `현장 수납` / `미수로 두기`, 차량 예비권으로 리프트권 추가 |
...
`docs/46` · `docs/51` · `docs/44` · `docs/45` · `docs/34`에 흩어져 있던 것을 한곳에 모았다. 3절(N1~N20)은 이번에 새로 찾은 것이라 여기서는 빼고 겹치는 것만 표시했다.
...
| D02 · D12 기사 업무 처리 | `예약금 N원 미수` 안내 띠 — 사용자 보류. 2-9의 기사 금액 자료가 생기면 풀림(우리 매장은 리프트권 값을 미리 받으므로 `미수` 안내로 일반화) |
...
- **나눠서 결제 S08R**(`docs/44` 6-1): 품목·수량을 골라 한 묶음씩 결제. 카드 단말을 붙일 때 구현. 접수 확정 창에 입구가 없고 높이 여유가 적다 — N4의 창 재배치, 여러 팀 한 번에 수납(N19)과 같은 창에서 설계하는 것이 좋다.
...
|---|---|---|---|
| 0 | 지금 | 사장님 답 반영 완료(6-5). 남은 23개는 확인 질문지로 받는다(6-6) | — |
| 1 | 10월 | **N1 배달 적재 연결**(기존 창 재사용이면 시안 없이 가능, 결함 수정 성격) → **N2 반납 장소 줄**(숙소 두 단계는 기존 구역·장소 구조, 설정 입력만) → **N12 순서 바꾸기 · `빨리 확인`**(기사 태블릿 · 휴대폰 · 포스, 배달 · 수거 공통) → **N3 수거 목록**(반납 타임별 · 인쇄 · 기사 검색) → **N4 칸별 `나중에` + 선입금 방식 설정** → **N19 여러 팀 한 번에 수납** → 작은 수정 N6 · N7 · N8 · N9 | 약 9장 · 27 크레딧 |
| 2 | 11월(연습 기간) | **N20 배달 현장 수납** → N17 의류 · 헬멧 교환 → N10 고객 찾기 → N11 방문 결과 → `사용 기능` 설정 → **관찰 시험 후 고치기** | 약 5장 · 15 크레딧 |
| 3 | 시즌 중 · 뒤 | 4-1 모양 차이(M04부터), 인쇄 나머지, 나눠서 결제(카드 단말 · N19와 같은 창), 관리자 콘솔(로그인 먼저), 예전 코드 정리, **다른 매장용: N5 강습팀 · 예약금 금액 방식** | 필요할 때 |

## 6. 사장님께 확인할 것

**2026-09-23 답을 받았다(6-5). 남은 질문은 확인 질문지(6-6)로 받는다.** 아래 6-1~6-4는 처음 여쭤본 내용을 기록으로 그대로 둔다.
...
### 6-5. 사장님 답 (2026-09-23)

| 질문 | 답 | 정해진 것 |
|---|---|---|
| 1 강습 | 매장마다 다르다. 우리 매장은 안 쓰지만 다른 곳은 쓸 수 있다. 스키학교보다 타 샵이나 강습팀에 맡기는 경우가 많다 | `사용 기능`으로 켜고 끄는 기능 + `강습팀` 목록(2-5). 우리 매장은 끔. 시즌 뒤(N5) |
| 2 야간 수거 | 서는 곳이 매번 다르고, 조기 반납 손님이 많아 왔다 갔다 한다. 수거 순서는 차량에서도 바꾸고, 매장으로 들어온 반납 요청은 포스에서 `빨리 확인` 같은 버튼으로 맨 위에 고정하거나 순서를 바꾼다. 배달도 같다 | 기사 · 매장 양쪽 순서 변경, `빨리 확인` = 맨 위 고정 + 알림(2-4 · N12). 수거 목록은 반납 타임별(N3) |
| 3 예약금 | 매장마다 다르다. 우리는 리프트권 값을 대부분 미리 다 받는다. 예약금만 받는 곳도 있다. 당일 끊어 둔 권의 취소 환불도 매장 판단 | 매장 설정 `선입금 방식`(리프트권 값 전부 / 예약금 / 안 받음), 취소 때 환불 여부 선택(2-6 · N4) |
| 4 결제 시점 | 보통 빌릴 때 받는다. 이따가 다른 팀이 결제하거나, 가족 단위 여러 팀이 마지막에 한 번에 내기도 한다 | `결제할 팀` 표시 + `한 번에 수납`(2-8 · N19) |
| 5 리프트권 | 리프트권은 리조트에서 미리 끊어 두었다가 그대로 전달하므로, 당일 대량 노쇼를 막으려고 값을 미리 받는다 | 3번과 같은 결론. 처음 질문(손님이 **리프트권만 먼저 돌려줄 때** 환불인지 재사용인지)은 뜻이 다르게 전달돼 질문지 2번으로 다시 묻는다 |
| 6 숙소 | 많다. 솔마을(한솔동 · 두솔동 · 세솔동 · 네솔동), 꽃마을(들국화 · 백합 · 민들레 · 개나리 · 해바라기 · 코스모스) 등 — 대표를 고르고 세부를 고르면 된다 | **지금 구조로 된다**: 구역 = 솔마을 · 꽃마을, 장소 = 동 이름. 체험판에 구역 6개를 넣어 장소 창을 확인(1024×600). 나머지 숙소는 질문지 19번 |
| 7 교환 | 옷 · 헬멧도 교환해 준다 | N17을 중간으로 올림 |
| 8 배달 때 돈 | 가끔 있다(리프트권 추가 등). 미수로 두고 매장에서 받기도 한다 | 기사 화면 미수 표시 · 현장 수납 · 리프트권 추가(2-9 · N20) |

### 6-6. 남은 질문 — 확인 질문지

- [확인 질문지](https://claude.ai/artifact/5tFWna6a1K6m9Z6iroGWRE): 23개, 대부분 체크만 하면 된다(숙소 이름 한 칸만 글자). 누르면 바로 저장되고(artifact db `answers/q01`~`q23` · `{picks, note, updatedAt}`), 저장이 안 되는 화면에서는 `답 복사`로 보낼 수 있다. 답이 모이면 Claude가 `ArtifactData`로 `answers`를 읽어 이 문서에 반영한다.
- 묶음: 우리 매장에서 쓰는 것(1) · 리프트권(2~5) · 결제(6~8) · 차량(9~15) · 배달 현장(16~17) · 교환(18) · 숙소(19) · 기계 · 인쇄(20~23).
- 부모님 확인용 [하루 흐름](https://claude.ai/artifact/Y2Myr96ySYeFsgTCVYcWP1) 그림도 이 답에 맞춰 고쳤다(2판).
- 두 페이지 모두 비공개다. 보여 줄 사람에게는 페이지의 공유 메뉴로 공유해야 한다. 질문지는 저장 기능 때문에 같은 조직 계정만 답을 저장할 수 있고 외부 초대는 보기만 된다 — **사용자 기기로 같이 보며 체크하는 것**이 가장 쉽다.
...
- 숙소 두 단계 선택은 체험판(메모리 자료)에 구역 6개(만선 · 설천 · 솔마을 · 꽃마을 · 가족호텔 · 기타)를 넣어 새 접수의 장소 창에서 확인했다. 구역 버튼 112px 6개, 잘린 글자 없음.
- 확인 질문지 저장 공간: 게시 직후 `answers` 컬렉션 0건을 확인했다(2026-09-23).
- 이 문서와 인계 문서 · README의 연결 줄 외에 저장소 파일 변경 없음. 사용자 파일 `docs/25-label-printer-purchase-notes.md`는 건드리지 않았다.
