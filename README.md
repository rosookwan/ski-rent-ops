...
작성일: 2026-09-07 · 문서 버전: 0.8 · 프로젝트명: `ski-rent-ops`
...
현재 산출물은 프로젝트 문서와 첫 인터랙티브 디자인 시안입니다. 시안은 매장 접수와 차량 간단·자세한 모드, 다일 계산, 상태 전환을 샘플 데이터로 보여줍니다. 실제 고객 데이터 저장, 기기 간 실시간 통신, 문자·전화·결제 연결, 서버 배포는 아직 없습니다.

0.8에서는 거래처 장부·렌탈현황·설정·사전 입력·준비표·QR 안내의 화면 구성안을 추가했습니다. 신규 페이지는 설계 단계이며 기존 인터랙티브 시안의 구현 범위는 그대로입니다.
...
| 사용자 요청 · 화면 설계 | 타 스키샵 리프트권 매입·판매와 장비 빌려오기·빌려주기 장부, 렌탈현황, 전체 상품·요금·권종 설정 |
| 사용자 요청 · 화면 설계 | 고객용 사전 사이즈 입력, 폼 설정·발송·응답 관리, 날짜별 준비표, 매장 안내 페이지와 QR |
