...
운영·직원에서는 반납 타임 순서를 위로·아래로 바꾸고, 직원과 기사님을 별도로 추가·수정·삭제할 수 있습니다. 매장 대표번호는 고객 안내에, 기사님 번호와 담당 차량은 배달·수거 및 차량의 연락처 목록에 표시합니다. 상호 변경은 안내·견적·문자 미리보기에도 반영됩니다. `test:operations`는 순서 변경·직원·연락처·입력 검증·기존 접수 보존·화면 크기·새로고침 초기화 7개 흐름을 확인합니다. 직원 등록은 체험용 명단 관리이며 실제 로그인 계정이나 권한을 발급하지 않습니다.

`test:notifications`는 알림·수신·확인·권한·재배정·DB 마이그레이션·저장 실패 롤백·두 HTTP 클라이언트의 12개 검사를 수행합니다. `test:notifications:ui`는 알림함·요청·접수값 보존·움직임 줄이기·낮은 화면·외부 변경 요청 없음 등 11개 흐름을 검사합니다. 실제 30초 반복과 확인 후 31초간 추가 재생이 없는지도 확인하므로 브라우저 검사는 1분 이상 걸립니다. 자동 검사는 재생 호출뿐 아니라 오디오 신호와 타이머를 확인하며, 실제 PC·탭 S6 스피커에서 들리는지 확인한 결과와는 구분합니다. 알림을 모두 읽은 뒤의 시험 재생, 보이는 화면에서 포커스만 벗어난 경우, 종 아이콘의 가로·세로 중앙 정렬도 검사합니다.
