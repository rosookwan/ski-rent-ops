...
`test:notifications`는 알림·수신·확인·권한·재배정·DB 마이그레이션·저장 실패 롤백·두 HTTP 클라이언트의 12개 검사를 수행합니다. `test:notifications:ui`는 알림함·요청·접수값 보존·움직임 줄이기·낮은 화면·외부 변경 요청 없음 등 10개 흐름을 검사합니다. 실제 30초 반복과 확인 후 31초간 추가 재생이 없는지도 확인하므로 브라우저 검사는 1분 이상 걸립니다. 자동 검사는 재생 호출과 타이머를 확인하며, 탭 S6에서 실제로 소리가 들리는지 확인한 결과와는 구분합니다.
