...
(async () => {
  ...
  try {
    await test('rental exchange request connects POS loading, vehicle delivery, old pickup, damage and final rental return', async () => {
      ...
      assert.equal((await latest()).units[0].receivedAt, undefined); assert.equal((await order()).items[0].customerQuantity, 2);
      ...
      await click('고객 직접반납 받음'); for (const name of ['스키','스키','의류','의류','헬멧']) await click(name + ' 수량 늘리기'); await click('직접반납 처리'); assert.equal((await order()).totals.customerQuantity, 0);
