...
def build():
    ...
    for name in ['pos-shell.css', 'pos-orders.css', 'pos-fulfillment.css', 'pos-adjustments.css', 'pos-workflow-windows.css', 'pos-keypad.css', 'pos-management.css']:
        if (APP / name).exists():
            fragment += '<style>\n' + (APP / name).read_text() + '\n</style>\n'
    for name in MODULES:
        if (APP / name).exists():
