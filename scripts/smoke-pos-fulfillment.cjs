...
(async () => {
  ...
  const openRecords = async () => { await action('pos-problems').click(); await click('전체 처리 이력'); };
  const exchangeSizes = async (oldSize, newSize) => {
    await click('현재 장비·규격'); await dialog().getByLabel('현재 규격', { exact: true }).fill(oldSize); await click('선택 적용');
    await action('pos-a2-options', 'size').click(); await click('직접 입력'); await dialog().getByLabel('새 규격', { exact: true }).fill(newSize); await click('선택 적용');
  };
  ...
  async function layout(name) {
    const result = await frame.evaluate(() => {
      ...
      const summary = modal?.querySelector('.pos-a2-summary'), error = modal?.querySelector('.pos-error:not([hidden])');
      if (summary?.getClientRects().length && error && summary.getBoundingClientRect().bottom > error.getBoundingClientRect().top + 1) scroll.push('adjustment summary overlaps error');
  ...
  try {
    ...
    await test('cancelling an unissued prepared item leads to preparation release before cancelling only that item', async () => {
      ...
      await openRecords(); await click('미도착·배달 취소'); await click('이 미지급분 취소');
    ...
    await test('dispatch uses selected quantities and vehicle 2; cancellation unloads before cancelling the unissued line', async () => {
      ...
      await openRecords(); await click('미도착·배달 취소'); await click('내리고 배달 해제'); await click('내린 물품 확인·배달 해제');
    ...
    await test('period extension and collection rescheduling change only the chosen line', async () => {
      ...
      await action('pos-schedule-line', 'ski-today').click(); await click('2호 차량'); await layout('schedule-600'); await click('확인하고 저장');
    ...
    await test('lost stock is excluded from normal return and found confirmation restores it to inspection', async () => {
      ...
      await openRecords(); await click('교환·분실·파손'); await click('상태·분실 확인'); await action('pos-condition-asset', assetId).click();
      ...
      await click('모두 받음'); assert.match(await dialog().locator('#so-dialog-title').innerText(), /반납 확인 1개/); await click('모두 반납 확정');
      await openRecords(); await click('교환·분실·파손'); await click('상태·분실 확인'); await click('실물 발견·매장 확인'); await click('실물 발견·매장 확인');
    ...
    await test('shop exchange records preparation, new delivery and old receipt before the replacement returns normally', async () => {
      ...
      await openRecords(); await click('교환·분실·파손'); await click('교환');
      await exchangeSizes('160', '165'); await layout('exchange-request-600');
    ...
    await test('a partial receipt correction allows the same selected quantity to be restored through its earlier collection', async () => {
      ...
      await openRecords(); await openCorrection(receipt); await dialog().getByLabel('되돌릴 수량', { exact: true }).fill('1'); await layout('partial-correction-600'); await click('원래 위치로 정정');
    ...
    await test('partial return extends only the one remaining customer asset with its own period and extra charge', async () => {
      ...
      assert.equal(await dialog().locator('[data-pos-input="fulfillment-amount"]').inputValue(), '10000'); assert.match(await dialog().innerText(), /남은 1개만 이용 연장/); await click('확인하고 저장');
    ...
    await test('A2 calendar recalculates extension and nested edits preserve the original revision', async () => {
      await fixture(); await click('준비·지급하기'); await click('선택 물품 지급 확정'); await click('기간·수거 변경');
      const before = await snapshot(); await action('pos-extend-line', 'ski-today').click(); await click('달력'); await layout('a2-extension-calendar-600');
      const date = await frame.evaluate(() => { let date = SkiOps.posData.today; for (let i = 0; i < 3; i++) date = SkiWorkflowCommon.nextDate(date); return date; });
      while (!await action('pos-a2-date', date).count()) await action('pos-calendar-move', '1').click();
      await action('pos-a2-date', date).click();
      assert.equal(await dialog().locator('[data-pos-input="fulfillment-amount"]').inputValue(), '60000');
      await click('취소'); assert.deepEqual(await snapshot(), before);
      await action('pos-extend-line', 'ski-today').click();
      ...
      await click('금액·사유 확인'); await dialog().getByLabel('이번 추가 청구액 (원)', { exact: true }).fill('15000'); await click('선택 적용');
      await click('확인하고 저장'); assert.match(await dialog().getByRole('alert').innerText(), /다른 처리|최신 수량/); await layout('a2-extension-stale-600'); assert.deepEqual(await snapshot(), changed);
    });
    await test('A2 next-morning selection applies the day offset once and saves only the chosen collection line', async () => {
      await fixture(); await click('준비·지급하기'); await click('선택 물품 지급 확정'); await click('기간·수거 변경'); await action('pos-schedule-line', 'ski-today').click();
      const initialDate = await dialog().locator('[data-pos-input="fulfillment-date"]').inputValue();
      await action('pos-a2-options', 'time').click(); await action('pos-a2-pick', 'next-morning').click();
      const nextDate = await dialog().locator('[data-pos-input="fulfillment-date"]').inputValue(); assert.notEqual(nextDate, initialDate);
      await action('pos-a2-pick', 'next-morning').click(); assert.equal(await dialog().locator('[data-pos-input="fulfillment-date"]').inputValue(), nextDate);
      await action('pos-a2-options', 'time').click(); assert.equal(await action('pos-a2-pick', 'next-morning').getAttribute('aria-pressed'), 'true'); await action('pos-a2-pick', 'afternoon').click();
      assert.equal(await dialog().locator('[data-pos-input="fulfillment-date"]').inputValue(), initialDate);
      await action('pos-a2-options', 'place').click(); await click('직접 입력'); await dialog().getByLabel('수거 장소', { exact: true }).fill('고객과 확인한 주차장'); await click('선택 적용'); await click('2호 차량'); await click('확인하고 저장');
      const result = await order(), plan = result.lines[0].returnPlan;
      assert.equal(plan.date, initialDate); assert.equal(plan.time, '16:30'); assert.equal(plan.place, '고객과 확인한 주차장'); assert.equal(plan.vehicleId, 'demo-van-2'); assert.equal(result.lines[1].returnPlan.method, 'direct');
    });
    await test('A2 paged problem selection records loss only after the existing asset confirmation', async () => {
      await fixture(Array.from({ length: 8 }, (_, index) => ({ id: 'problem-' + index, sku: 'ski', quantity: 1 })));
      await click('준비·지급하기'); await click('선택 물품 지급 확정'); const before = await snapshot();
      await action('pos-problems').click(); await click('분실'); await click('다음'); await layout('a2-problem-page-2-600'); await action('pos-problem-line', 'problem-4').click(); await click('다음 · 분실');
      const assetId = (await order()).lines[4].customerAssetIds[0]; await action('pos-condition-asset', assetId).click();
      assert.equal(await dialog().getByLabel('확인한 상태', { exact: true }).inputValue(), 'lost'); assert.equal(await dialog().getByLabel('처리 사유', { exact: true }).inputValue(), '고객 분실 신고');
      assert.deepEqual(await snapshot(), before); await click('취소'); assert.deepEqual(await snapshot(), before);
      await action('pos-condition-asset', assetId).click(); await click('확인하고 저장');
      const after = await snapshot(); assert.equal(after.assets.find(asset => asset.id === assetId).condition, 'lost');
      assert.deepEqual(after.assets.filter(asset => asset.id !== assetId), before.assets.filter(asset => asset.id !== assetId));
    });
    await test('A2 quantity correction includes a fully returned line and keeps whole-order money and cancellation routes', async () => {
      await fixture(); await click('준비·지급하기'); await click('선택 물품 지급 확정'); await click('모두 받음'); await click('모두 반납 확정'); const before = await snapshot();
      await action('pos-problems').click(); await click('수량 정정'); await action('pos-problem-line', 'coat-today').click(); await click('다음 · 수량 정정');
      const ids = await action('pos-correct-movement').evaluateAll(nodes => nodes.map(node => node.dataset.id));
      const coatIds = (await order()).lines[1].shopAssetIds;
      const movements = await frame.evaluate(() => SkiOps.posData.history.movements); assert.ok(ids.length > 0); assert.ok(ids.every(id => movements.find(row => row.id === id).assetIds.some(id => coatIds.includes(id))));
      await go('order-detail'); await action('pos-problems').click(); await click('금액 정정'); assert.match(await dialog().innerText(), /접수 전체/); await click('다음 · 금액 정정'); assert.match(await dialog().innerText(), /수납|환불/); await click('취소');
      await action('pos-problems').click(); await click('접수 취소'); await click('다음 · 접수 취소'); assert.equal(await dialog().count(), 0); assert.match(await frame.locator('#so-page').innerText(), /미도착·배달 취소/); assert.deepEqual(await snapshot(), before);
    });
    await test('A2 registered sizes and component confirmation lead to a vehicle request without moving any equipment', async () => {
      await fixture([{ id: 'ski-today', sku: 'ski', quantity: 1 }]);
      await frame.evaluate(async () => { const D = SkiOps.posData, asset = D.snapshot.assets.find(asset => asset.sku === 'ski' && asset.size === '160' && asset.location.kind === 'shop' && asset.condition === 'ready');
        for (const size of ['145', '150', '155', '165', '170', '175', '180']) await D.execute('stock.receive', { sku: 'ski', quantity: 1, size });
        await D.execute('ops.prepare', { orderId: 'pos-smoke', lineItems: [{ lineId: 'ski-today', assets: [{ assetId: asset.id, size: '160' }] }] }); SkiOps.render(); });
      await click('준비·지급하기'); await click('선택 물품 지급 확정'); await openRecords(); await click('교환·분실·파손'); await click('교환'); const before = await snapshot();
      await click('교환 내용 확인'); await layout('a2-exchange-missing-size-600'); await page.screenshot({ path: output + '/exchange-missing-size.png' });
      assert.equal(await action('pos-a2-pick', '160').isDisabled(), true); await action('pos-a2-pick', '165').click();
      await action('pos-a2-edit', 'method').click(); await click('파손 교체'); await click('선택 적용'); assert.equal(await action('pos-a2-pick', '160').isDisabled(), false);
      await action('pos-a2-pick', 'ski-boots').click(); await click('교환 내용 확인'); assert.match(await dialog().getByRole('alert').innerText(), /현재 규격과 새 규격/);
      await exchangeSizes('265', '270'); await action('pos-a2-edit', 'method').click(); await click('차량으로 교환'); await click('사이즈 변경'); await click('선택 적용');
      assert.deepEqual(await snapshot(), before); await click('교환 내용 확인'); await click('2호 차량'); await layout('a2-exchange-visit-600'); await click('교환 방문 요청 생성');
      const after = await snapshot(), exchange = after.exchanges.find(exchange => exchange.orderId === 'pos-smoke');
      assert.equal(exchange.kind, 'ski-boots'); assert.equal(exchange.oldSize, '265'); assert.equal(exchange.newSize, '270'); assert.equal(exchange.visit.vehicleId, 'demo-van-2'); assert.equal(exchange.status, 'open');
      const baseId = (await order()).lines[0].customerAssetIds[0]; assert.equal(after.assets.find(asset => asset.id === baseId).location.kind, 'customer'); assert.ok(exchange.units.every(unit => !unit.deliveredAt && !unit.receivedAt));
    });
