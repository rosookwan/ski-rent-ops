  ...
  try {
    ...
    for (const [width, height] of sizes) {
      ...
      if (width === 1024 && height === 600) {
        ...
        await field('differenceReason').fill('검사 중 현금 실사'); await action('pos-closing-next').click(); await finishHandover();
        const changed = await frame.evaluate(async () => { await SkiOps.posData.execute('stock.receive', { sku: 'helmet', quantity: 1 }); return SkiOps.posData.snapshot; });
        ...
        await action('pos-closing-start').click(); await field('countedCashWon').fill('1'); await field('differenceReason').fill('검사 중 현금 실사'); await action('pos-closing-next').click(); await finishHandover(); await action('pos-closing-save').click(); assert.equal(await dialog().count(), 0); assert.equal((await snapshot()).closings.length, changed.closings.length + 1);
