...
(async () => {
  ...
  try {
    ...
    await test('yesterday returns continue across several collect/unload trips without closing the day', async () => {
      ...
      const collect = async n => { await card.getByRole('button', { name: '수거 수량 입력', exact: true }).click(); await frame.getByLabel('왕복 스키 수량', { exact: true }).fill(String(n)); await click('선택 수량 확인'); };
      ...
      assert.equal(await card.getByRole('button', { name: '수거 수량 입력', exact: true }).count(), 0);
      ...
      await click('추가로 싣기'); await frame.getByLabel('왕복 스키 수량', { exact: true }).fill('1'); await click('선택 수량 확인');
